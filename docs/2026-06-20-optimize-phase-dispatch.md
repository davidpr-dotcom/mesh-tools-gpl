...
## 8. Outcome + deferred optimizers (2026-06-21)

- **cow result:** compose-on-stall (localDescent + cornerScale, incl. mild expansion) reached **80.609** vs 80.84 single-lever — the composition unstuck localDescent (the "combine when one stalls" payoff). The residual is a **structural thin-feature/crevice** (STL diagnostic `Zefra/tools/2026-06-21-cow-ear-curvature-diag.py`: 155-175° surface turn within ~one base cell, coarse 0.035 STL > 0.018 base faces) — **not** refinable, and not a representative aero defect. Accepted as a localized test-geometry floor; gap-aware fewer-layers is the lever only if a *real* geometry shows the signature.
- **`globalSmooth` / `skewTargeted` — DEFERRED (no measured target).** Every stall measured is structural (cow thin-feature, bracket irreducible), not a local optimum a coordinated `globalSmooth` would escape — and compose-on-stall already gives cross-lever escape. nonOrtho is the binding metric everywhere; skew stays < the 4.0 limit (ahmed peaked 3.94, already gated; no skew checkMesh failures) → no `skewTargeted` target. Per prove-before-modify they remain registry placeholders: each slots in behind the same never-worse + active-set interface the moment a census surfaces its defect. Building either now would be dead code.
- **Stage-C optimize phase = COMPLETE:** localDescent + cornerScale + stall-driven pipeline + active-set + early-stop, all committed, never-worse, deterministic, extensible.
